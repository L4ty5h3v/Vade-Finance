...
import { fundInvoiceAction } from "@/lib/server/actions";
...
import { fundInvoiceSchema } from "@/lib/server/validators";
...
export async function POST(request: Request, ctx: { params: Promise<{ invoiceId: string }> }) {
  try {
    ...
    const parsed = fundInvoiceSchema.safeParse(body);
    if (!parsed.success) badRequest("Invalid fund payload");

    const { invoiceId } = await ctx.params;
    const updated = await fundInvoiceAction({
      invoiceId,
      user,
